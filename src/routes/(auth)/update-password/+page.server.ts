...
export let actions = {
  default: async ({ request, url, locals: { supabase } }) => {
    ...
    if (new_password !== password_conf) return fail(400,{message:"รหัสผ่านไม่ตรงกัน",ok:false})
    if (new_password?.length < 8) return fail(400,{message:"รหัสต้องมีอย่างน้อย 8 ตัวอักษร",ok:false})

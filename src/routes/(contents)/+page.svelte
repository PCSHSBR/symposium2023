<script lang="ts">
	...
	onMount(() => {
		...
		function updateAnimation() {
			...
			document.querySelector('.title').style.top = `${
				(scaledNumber(0.5, 1, hero_section_percent, 0, 30) - 20) * -1
			}%`;
			document.querySelector('.title').style.opacity = `${
				(scaledNumber(0.5, 1, hero_section_percent, 0, 1) - 1) * -1
			}`;
		}
		updateAnimation();
	});
...
<div class="hero-sticky h-[200vh]" bind:this={hero_sticky}>
	...
	>
		...
		<div>
			<div class="title absolute px-16 md:px-32">
				<h1
				...
				<p class="sub-title mix-blend-difference invert dark:mix-blend-difference dark:invert-0">
